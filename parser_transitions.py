...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S' and len(self.buffer) > 0:
            ...
            src = self.stack[-1]     # last element is the source
            dst = self.stack.pop(-2) # second to last element is the destination
            self.dependencies.append((src, dst))
        elif transition == 'RA':
            src = self.stack[-2]     # second to last element is the source
            dst = self.stack.pop()   # last element is the destination
            self.dependencies.append((src, dst))
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for partial_parse, transition in zip(parses, transitions):
    ...
    dependencies = [iterator.dependencies for iterator in partial_parses]
    ### END YOUR CODE
